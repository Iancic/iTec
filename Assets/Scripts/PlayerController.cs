...
public class PlayerController : MonoBehaviour
{
    ...
    {   
        ...
        theRB.velocity = new Vector2(moveSpeed * Input.GetAxis("Horizontal_AD"), theRB.velocity.y);
        else
        theRB.velocity = new Vector2(moveSpeed * Input.GetAxis("Horizontal_1"), theRB.velocity.y);
        ...
        isGrounded = Physics2D.OverlapCircle(groundCheckPoint.position, .2f, whatIsGround);
                ...
                if (Input.GetButtonDown("Jump_1") || Input.GetButtonDown("Jump_F"))
                {
                    if (isGrounded)
                    {
